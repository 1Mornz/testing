...
import { seedUsers } from './authService.js';
...
const demoProvider = {
  ...
  quizQuestions: [
    {
      id: 'demo-q-1',
      label: 'Is the project located inside our service area?',
      type: 'yesno',
      required: true,
      options: [],
      preferredAnswers: ['yes'],
    ...
    },
    {
      ...
      type: 'multiple',
      required: true,
      options: ['Repair', 'Installation', 'Inspection only'],
      preferredAnswers: ['repair', 'installation'],
      disqualifyingAnswers: ['inspection only'],
      sortOrder: 1,
    },
  ],
...
const demoRequests = [
  {
    ...
    budgetRange: '$250 - $500',
    status: 'reviewing',
    qualification: { status: 'qualified', score: 2, matched: 2, disqualified: false },
    quizAnswers: [
      { questionId: 'demo-q-1', questionLabel: 'Is the project located inside our service area?', answer: 'yes' },
      { questionId: 'demo-q-2', questionLabel: 'What kind of project is this?', answer: 'Repair' },
    ],
    deposit: {
      enabled: true,
      required: false,
  ...
  },
  {
    ...
    budgetRange: '$100 - $250',
    status: 'new',
    qualification: { status: 'needs_review', score: 0, matched: 0, disqualified: false },
    quizAnswers: [
      { questionId: 'demo-q-1', questionLabel: 'Is the project located inside our service area?', answer: 'yes' },
      { questionId: 'demo-q-2', questionLabel: 'What kind of project is this?', answer: 'Installation' },
    ],
    deposit: {
      enabled: true,
      required: false,
...
export async function seedDemoData({ force = false } = {}) {
  ...
  if (force || !providers.length) await writeJson('providers.json', [demoProvider]);
